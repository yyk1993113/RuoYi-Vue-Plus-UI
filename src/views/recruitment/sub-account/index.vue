    ...
    >
      <div v-loading="paymentAccountLoading" class="payment-account-dialog">
        ...
        <el-table :data="paymentAccountRows" row-key="accountId" class="payment-account-table">
          ...
          <el-table-column label="结算通道名称" min-width="180">
            <template #default="{ row }">
        ...
        <el-form class="payment-account-add-form" label-width="96px" @submit.prevent>
            ...
            <div class="payment-account-selector-row">
              <el-select v-model="paymentAccountProfileAccountId" placeholder="请选择结算通道" @change="selectPaymentAccountProfile">
                <el-option
                  ...
                  :label="`${settlementAccountName(item.accountName) || '结算通道名称待补充'}（${item.subjectCompanyName || '结算企业待补充'}）`"
          ...
          <el-form-item label="结算通道名称">
            <el-input v-model="paymentAccountEditor.accountName" maxlength="64" placeholder="请输入结算通道名称" />
          </el-form-item>
...
async function loadAuditApplicationDetail(row: SettlementSubAccountVO) {
  ...
  try {
    // 本次提交的材料只允许来自白名单申请快照，不能混入企业表当前资质。
    const snapshotResponse: any = await getSettlementQualification(row.applicationId);
    ...
    auditMaterialFiles.value = await resolveMaterials(snapshot, row.companyId);
  } catch (error) {
...
async function openDetail(row: SettlementSubAccountVO, tab: DetailTab = 'application') {
  ...
  const latestCert = hasQualificationSnapshot ? { ...qualificationSnapshot } : {};
  currentCompany.value = company;
  currentCert.value = latestCert;
  seedDetailQualificationIds(latestCert);
  const materialSource = hasQualificationSnapshot ? { ...qualificationSnapshot } : {};
...
function validatePaymentAccountProfile(
  ...
  if (!draft.accountName) return '请输入结算通道名称';
...
async function confirmDeletePaymentAccount(row: SettlementPaymentAccount) {
  ...
  const confirmed = await ElMessageBox.confirm(
    `确认删除结算账户“${settlementAccountName(row.accountName) || '未填写结算通道名称'}”吗？删除后该账户将不再显示，且不可直接恢复。`,
    '删除结算账户',
    {
      type: 'warning',
  ...
  try {
    ...
    if (incompleteAccount) {
      return ElMessage.warning(
        `请先完善结算账户“${settlementAccountName(incompleteAccount.accountName) || '结算通道名称待补充'}”的结算通道名称与结算企业资料`
      );
    }
